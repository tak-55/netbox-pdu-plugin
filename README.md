...
On an outlet or inlet detail page, click **Push Name to PDU** to write the name stored in NetBox to the physical device. The corresponding `PowerOutlet.label` or `PowerPort.label` on the connected NetBox device is also updated.
...
# Run tests (Docker)
docker compose exec netbox bash -c "cd /opt/netbox/netbox-pdu-plugin && uv run pytest"
...
アウトレットまたはインレットの詳細ページで **Push Name to PDU** をクリックすると、NetBox に保存されている名前を物理デバイスに書き込みます。接続先デバイスの `PowerOutlet.label` / `PowerPort.label` も同時に更新されます。
